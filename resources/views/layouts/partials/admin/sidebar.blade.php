...
<div class="page-sidebar">
    <ul class="list-unstyled accordion-menu">
        ...
        <li @if (in_array($currentUrl, $colLinkManagementMenu)) class="open" @endif>
            <a href="{{ $item->link_management_menu }}"><i data-feather="{{ $item->icon_management_menu }}"></i>{{ $item->nama_management_menu }}<i class="fas fa-chevron-right dropdown-icon"></i></a>
            <ul>
                <?php
                $subMenu = $item->membawahi_menu_management_menu;
                $getMenu = explode(',', $subMenu);
                ...
                <li class="{{ $currentUrl == $sub_item->link_management_menu ? 'active-page' : '' }}"><a style="{{ $currentUrl == $sub_item->link_management_menu
                                            ? 'border-radius: 10px; background-color: #f3f6f9; color: #7888fc; font-weight: 500;'
                                            : '' }}" href="{{ url($sub_item->link_management_menu) }}"><i class="{{ $sub_item->icon_management_menu }}"></i>{{ $sub_item->nama_management_menu }}</a>
                </li>
                @endif
                @endforeach
            </ul>
        ...
        <li class="{{ $currentUrl == $item->link_management_menu ? 'active-page' : '' }}">
            <a href="{{ url($item->link_management_menu) }}" class="{{ $item->link_management_menu == '/logout' ? 'btn-logout' : '' }}"><i data-feather="{{ $item->icon_management_menu }}"></i>{{ $item->nama_management_menu }}</a>
        </li>
        @endif
        @endif
        @endforeach
