<script>
    $(document).ready(function(e) {
        var tableKoordinator = $('#dataTableKoordinator').DataTable({
            ...
            ajax: {
                url: "{{ route('admin.koordinator.usersKoordinator') }}",
                dataType: 'json',
                type: 'get',
            },
            columns: [{
        ...
        $('#dataTableKoordinator tbody').on('click', 'td.details-control .btn-show-users', function() {
            var tr = $(this).closest('tr');
            var row = tableKoordinator.row(tr);
        ...
        var table = $('#dataTable')
            .DataTable({
                ...
                searchHighlight: true,
                ajax: {
                    url: "{{ route('admin.koordinator.index') }}",
                    type: 'get',
                    data: {
                        tps_id: "{{ $tps_id }}"
                    }
        ...
        $(document).on('click', '.btn-add', function(e) {
            ...
            let url = "{{ url('/') }}";
            $('.form-submit').attr('action', url + '/admin/koordinator');
        ...
        function setUsersId(setData = {}) {
            var users_id = $('.users_id_select').select2({
                theme: 'bootstrap-5',
                ajax: {
                    url: `{{ url('/admin/koordinator/selectKoordinator') }}`,
                    dataType: 'json',
        ...
        $(document).on('click', '.btn-edit', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let url = "{{ url('/') }}";
                    $('.form-submit').attr('action', url + '/admin/koordinator/' + result.id);
        ...
        function saveSessionCheck(setData = {}) {
            let url = "{{ url('/') }}";
            $.ajax({
                url: `${url}/admin/koordinator/saveSession`,
                dataType: 'json',
                type: 'get',
                data: setData,
                success: function(data) {
