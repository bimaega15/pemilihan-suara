        ...
        <div class="carousel-inner">
            ...
            @foreach ($banner as $index => $item)
                <div class="carousel-item {{ $index == 0 ? 'active' : '' }}">
                    <img class="d-block w-100" src="{{ asset('upload/banner/' . $item->gambar_banner) }}"
                        alt="{{ $item->gambar_banner }}">
                    <div class="carousel-caption d-none d-md-block"
                        style="top: 25%; left: 15%; text-align: left; z-index: 2;">
                        <h3 class="color-orange w-75">{{ Check::truncateText($item->judul_banner, 30, '...') }}</h3>
                        <p class="font-weight-bold w-75">{{ Check::truncateText($item->keterangan_banner, 80, '...') }}
                        </p>
                    </div>
                </div>
            @endforeach
