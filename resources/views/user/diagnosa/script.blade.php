<script>
    $(document).ready(function() {
        ...
        function loadDataKuisioner(url = "{{ url('/users/diagnosa/') }}") {
            $.ajax({
                url,
                method: 'get',
                dataType: 'json',
                success: function(data) {
                    ...
                    <div class="d-flex justify-content-end">
                        <button type="button" data-href="${kuisioner.prev_page_url}" class="btn btn-secondary mr-3 shadow-sm px-5 btn-prev"><i class="fas fa-arrow-left font-weight-bold"></i></button>`;
                    } else {
                        outputLink =
                            `
                    <div class="d-flex justify-content-end">
                        <button type="button" class="btn btn-secondary mr-3 shadow-sm px-5 btn-biodata"><i class="fas fa-arrow-left font-weight-bold"></i></button>`;
                    }


                    if (kuisioner.next_page_url != null) {
                        outputLink += `
                        <button type="button" data-href="${kuisioner.next_page_url}" class="btn btn-secondary shadow-sm px-5 btn-next" ${kuisioner.next_page_url == null ? 'disabled' : ''}><i class="fas fa-arrow-right font-weight-bold"></i></button>
                    </div>
                    `;
                    } else {
                        let url = "{{ url('/') }}";
        ...
        $(document).on('click', '.check-data-testing', function() {
            ...
            let url = "{{ url('/') }}";
            $.ajax({
                url: `${url}/users/diagnosa/storeSession`,
                type: 'get',
                dataType: 'json',
                data: {
                    kuisioner_id,
        ...
        function getStoreSession() {
            ...
            var output = null;
            $.ajax({
                url: `${url}/users/diagnosa/getStoreSession`,
                type: 'get',
                dataType: 'json',
                async: false,
                success: function(data) {
